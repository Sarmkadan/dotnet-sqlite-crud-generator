...
public sealed class BulkImportExportEngine<T> : IBulkTransferService<T> where T : class
{
    ...
    /// <summary>
    /// Initialises a new engine with audit trail support.
    /// </summary>
    /// <param name="repository">Repository providing data access for type <typeparamref name="T"/>.</param>
    /// <param name="exportService">Service used to serialise entities during export.</param>
    /// <param name="auditTrailService">Optional audit trail service for recording bulk operations.</param>
    /// <param name="userId">ID of the user performing the bulk operation (required if auditTrailService is provided).</param>
    /// <param name="options">
    /// Behavioral configuration. Defaults to <see cref="BulkTransferOptions.Default"/> when <c>null</c>.
    /// </param>
    public BulkImportExportEngine(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // Record bulk operation audit trail if service is available
        if (_auditTrailService != null && result.Succeeded > 0)
        {
            try
            {
                await _auditTrailService.RecordBulkOperationAsync(
                    typeof(T).Name,
                    sessionId,
                    OperationType.Import,
                    _userId ?? 0,
                    result.Succeeded,
                    null, // source - could be enhanced to track file/stream info
                    null, // checkpointId - could be enhanced to track checkpoint info
                    null, // reason
                    null, // ipAddress
                    cancellationToken);
            }
            ...
            {
                // Don't fail the bulk operation if audit logging fails
                Console.Error.WriteLine($"[BulkTransfer] Failed to record audit trail: {ex.Message}");
            }
        }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // Record bulk operation audit trail if service is available
        if (_auditTrailService != null && result.TotalExported > 0)
        {
            try
            {
                await _auditTrailService.RecordBulkOperationAsync(
